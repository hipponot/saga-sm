  ...
  async build(): Promise<BellSchedule> {
    ...
    const createInput: CreateCompleteScheduleInput = {
      ...
      dayLabelRuleSet: this.schedule.dayLabelRuleSet ? {
        ...
        dayOfWeekRules: this.schedule.dayLabelRuleSet.dayOfWeekRules?.map(rule => {
          const day = this.schedule.days.find(d => d.id === rule.scheduleDayId);
        ...
        patternBasedRules: this.schedule.dayLabelRuleSet.patternBasedRules?.map(rule => {
          const day = this.schedule.days.find(d => d.id === rule.scheduleDayId);
          if (!day) throw new Error(`Day not found for rule: ${rule.id}`);
          return {
            id: rule.id,
      ...
      variantRuleSet: this.schedule.variantRuleSet ? {
        ...
        defaultVariantName: (() => {
          const defaultVariant = this.schedule.variants.find(v => v.id === this.schedule.variantRuleSet!.defaultVariantId);
        ...
        exceptions: this.schedule.variantRuleSet.exceptions.map(exception => {
          const variant = this.schedule.variants.find(v => v.id === exception.variantId);
          if (!variant) throw new Error(`Variant not found for exception: ${exception.id}`);
          return {
            id: exception.id,
            date: exception.date,
