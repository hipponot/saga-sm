...
if [ "$SKIP_BUILD" != "true" ]; then
    ...
    # Add build metadata
    mkdir -p out
    echo "{\"buildTime\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"commit\":\"$(git rev-parse HEAD 2>/dev/null || echo 'unknown')\",\"branch\":\"$BRANCH_NAME\",\"amplifyBranch\":\"$AMPLIFY_BRANCH_NAME\",\"env\":\"$BUILD_ENV\",\"amplifyAppId\":\"$AMPLIFY_APP_ID\"}" > out/build-info.json
