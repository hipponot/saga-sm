...
test.describe('Connection Testing and Ping/Pong Functionality', () => {
    ...
    test('live event stream section is present', async ({ page }) => {
        ...
        await expect(page.getByText('No events yet. Test the API to see the live stream!')).toBeVisible();
    });
    ...
    test('statistics show initial values', async ({ page }) => {
        ...
        const statsSection = page.locator('.statsGrid, [class*="statsGrid"], [class*="statCard"]').first();
